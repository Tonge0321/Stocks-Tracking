...
# 2. 2026/1/9 完整股票數據庫 (包含正規市場與盤後期貨)
stock_db = {
    ...
    'NVDA':    { 'regular': 185.08, 'futures': 184.90, 'change_pct': -2.13 }, # 輝達
    'SOFI':    { 'regular': 27.72,  'futures': 27.85,  'change_pct': 2.67 },
    'EOSE':    { 'regular': 14.02,  'futures': 14.00,  'change_pct': 0.29 },
    'S':       { 'regular': 15.33,  'futures': 15.40,  'change_pct': -1.29 }, # SentinelOne
    'CRWV':    { 'regular': 77.09,  'futures': 76.80,  'change_pct': -0.12 }, # CoreWeave
    'VRT':     { 'regular': 160.73, 'futures': 161.50, 'change_pct': -6.30 }, # Vertiv
...
def run_full_update():
    ...
    for ticker in tickers_list:
        ...
        if stock:
            payload.append({
                "symbol": ticker,
                "price": f"{stock['regular']:.2f}",       # 主價格 (收盤)
                "futuresPrice": f"{stock['futures']:.2f}", # 期貨/盤後價
    ...
    try:
        ...
        if "Success" in response.text or "Updated" in response.text:
            print("\n✅ 更新成功！請檢查您的 Google Sheet。")
            print("您應該會看到 NVDA 價格為 185.08，下方期貨價為 184.90")
        else:
            print("\n❌ 更新可能失敗，請檢查錯誤訊息。")
